...
const int DIR_C=50;
const int PWM_C=6;
const int Max_speed_h=50; // maximum speed of hand grip          //----------------------------------------------Speed hand grip
             ...
             if (backward==HIGH)
              {
                ...
                if (digitalRead(start_point_switch)==HIGH)  {
                    ...
                    case MY_POSITION_T0:    // your hand is on the sensor
                      Serial.println("T0");
                      if (handguide_position<=80)
                          {
                                     
                                 //if (handguide_position>start_position-10 && handguide_position<=start_position+30) 
                                 backward_C();// Handgrip is　going  
                                 if (handguide_position >=start_position && handguide_position>start_position+5) handgrip_stop_backward();
                                    }
                                
                      break;
                    case MY_POSITION_T1:    // your hand is close to the sensor
                      Serial.println("   T1");
                        my_current_position=MY_POSITION_T2;
                      break;
                    case MY_POSITION_T2:    // 
                      Serial.println("         T2");
                      if (handguide_position>160 && handguide_position<170) forward_C();  // going reverse stops at 80
                      if (handguide_position<=135) 
                       { 
                         backward_A();
                         backward_B();
                       }
