                ...
                // Serial.println(forward);
                 ...
                 switch (my_current_position) {
                    case MY_POSITION_T0:    // your hand is on the sensor
                      Serial.println("T0");
                      if (handguide_position>=60 && handguide_position<=160)
                          {
                                     
                                 if (handguide_position>start_position-10 && handguide_position<=start_position+10) forward_C();// Handgrip is　going  
                                 if (handguide_position<105)
                                    {
                                      forward_A();
                                      forward_B(); // lift　is　going 
                                    }
                                 if (handguide_position<=105) handgrip_stop_forward(); // // handgrip stops
                    ...
                    case MY_POSITION_T2:    // 
                      Serial.println("         T2");
                      if (handguide_position>30 && handguide_position<=110) backward_C(); 
                      if (hall_A1_counter>100) stop();
                      if (handguide_position>110)
                      {
                        //stop();
...
// --------------------------------------backward--------------------------------------starts here            
                ...
                if (digitalRead(start_point_switch)==HIGH)  {
                    ...
                    case MY_POSITION_T0:    // your hand is on the sensor
                      ...
                      //if (handguide_position>=110)
                         // {
                                  digitalWrite(DIR_C, LOW);   
                                 //if (handguide_position>start_position-10 && handguide_position<=start_position+30) 
                                 analogWrite(PWM_C, 60);// Handgrip is　going  
                                 
                                 if (hall_A1_counter<=15) stop();
                                 
                                 
                                 if (handguide_position>start_position && handguide_position<start_position+5)
                                                  { 
                                                    //stop();
                                 ...
                                 //if (handguide_position <=start_position && handguide_position>start_position) analogWrite(PWM_C, 0); //handgrip_stop_backward();
                                   // }
                    ...
                    case MY_POSITION_T2:    // 
                      Serial.println("         T2");
                      if (handguide_position>110 && handguide_position<140) forward_C();  // going reverse stops at 80
                      if (handguide_position<=125) 
                       { 
                         backward_A();
                         backward_B();
                       }
                      if (handguide_position<=120) 
                      {
                        //stop();
          ...
          if (hall_A1_counter>=0&& hall_A1_counter<=30) my_current_position = MY_POSITION_T0;
          if (hall_A1_counter>30&& hall_A1_counter<=31) my_current_position = MY_POSITION_T1;
          if (hall_A1_counter>31&& hall_A1_counter<=150) my_current_position = MY_POSITION_T2;
